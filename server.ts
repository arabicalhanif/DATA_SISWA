  ...
  app.post("/api/ai/chat", async (req: express.Request, res: express.Response) => {
    ...
    const generateLocalFallbackAI = (msgs: any[], ctx: any): string => {
      ...
      if (query.includes("remedial") || query.includes("di bawah kkm") || query.includes("pendampingan") || query.includes("kurang") || query.includes("lemah")) {
        ...
        if (lowPerformers.length > 0) {
          ...
          return sapaan + "Luar biasa! Seluruh siswa yang terdaftar saat ini memiliki performa nilai di atas KKM standar (>= 75). Pembelajaran di kelas Anda berjalan sangat efektif! Tetap pertahankan metode pengajaran kreatif Anda.";
        }
      }
    ...
    try {
      ...
      if (currentRole === "guru" && teacherName) {
        // Simple name clean-up for natural call (e.g. "Ahmad Hanif, M.Pd." -> "Ahmad Hanif" or "Ibu Siti Aminah, S.Pd." -> "Ibu Siti")
        const cleanName = teacherName.replace(/(,?\s*[S|M]\.[P|K]d\.?)/g, "").trim();
...
1. SAPA GURU SECARA PERSONAL: Anda WAJIB menyapa Guru ini secara ramah, sopan, dan hangat sesuai namanya pada sela-sela obrolan (misalnya: "Bapak ${cleanName}", "Ibu ${cleanName}", atau "Pak ${cleanName.split(' ')[0]}"). Jadikan sapaan ini terasa alami di awal respon Anda atau saat memberikan rincian data. Contoh: "Halo Ibu Siti, senang sekali bisa mendampingi Anda hari ini...", atau "Tentu Bapak Ahmad, mari kita tinjau nilai anak-anak...".
2. NADA BAHASA SANGAT HANGAT DAN SENTUHAN EMPATI: Gunakan gaya bahasa Indonesia yang mengayomi, bersahabat, penuh sukacita, optimis, sejuk, dan memberikan apresiasi tulus atas jasa kepahlawanan pendidik mereka. Hindari respon yang terlalu dingin atau berjarak standar robotik! Rasanya siber-asisten pribadi yang loyal, penuh cinta kasih, dan suportif.
3. DATA JELAS, TRANSPARAN, DAN AKTUL: Klasifikasikan data siswa, nilai, rincian kkm, atau daftar remedial secara terstruktur dan jelas menggunakan format list bullet-points (•) atau penomoran yang rapi. Tunjukkan analisis statistik sederhana (rata-rata, tuntas / tidak tuntas) agar guru mudah memahaminya dalam sekali baca.`;
      } else if (currentRole === "admin") {
        identityInstructions = `
PENGGUNA YANG SEDANG BERBICARA DENGAN ANDA SAAT INI:
- Peran Pengguna: Administrator Utama (Super Admin)
- Nama Admin: ${teacherName || "MGMP BAHASA ARAB"}

ATURAN KOMUNIKASI DAN INTEGRASI NAMA ADMIN:
1. SAPA ADMIN SECARA PERSONAL: Sapa dengan hormat, hangat, dan sebutkan nama "MGMP BAHASA ARAB" secara khusus di sela respons Anda atas pengelolaan sistem.
...
ATURAN KOMUNIKASI UNTUK WALI MURID:
                  1. NADAN BAHASA PENUH KEDAMAIAN & KERJA SAMA: Sapa wali murid dengan sapaan hormat "Bapak/Ibu Wali Murid" yang sejuk. Gunakan tata bahasa yang sangat santun, penuh sukacita, hangat, serta membangun sinergi positif antara guru-sekolah di kelas dan orang tua di rumah.
                  2. ANALISIS YANG MEMUDAHKAN: Jelaskan performa akademik anak dengan penuh kejujuran data namun dibalut motivasi yang meyakinkan bahwa setiap siswa punya keunikan terbaik dan pasti bisa berkembang pesat jika terus didukung dengan penuh kegembiraan di rumah.`;
      }
...
1. SANGAT PROFESIONAL & VISIONER: Gunakan gaya bahasa Indonesia yang formal, profesional, taktis, bersahabat, visioner dan solutif. Anda adalah mitra strategis administrator pimpinan sekolah.
2. KOMUNIKASI SISTEM DAN AUDIT: Fokus pada kestabilan tata kelola dan sinkronisasi data sekolah. Jabarkan pangkalan data sekolah, guru, jadwal pelajaran, rombel kelas secara transparan dan tertata rapi.
...
1. SANGAT HANGAT, CERDAS, & MEMOTIVASI: Gunakan nada bicara yang ramah, hangat, penuh perhatian, dan membakar semangat belajar peserta didik. Berikan saran-saran pembelajaran berbasis teori mutakhir Kurikulum Merdeka atau Pembelajaran Berdiferensiasi.
2. METODOLOGI MENGAJAR & TATA BAHASA ARAB (NAHWU-SHOROF): Berikan ide-ide kreatif cara mengajar mufrodat Arab yang gembira untuk guru. Jelaskan kaidah Nahwu-Shorof secara mendalam, lengkap dengan analisis sintaksis (i'rab) bila ditanya.
3. DATA AKTUAL & TEPAT: Selalu gunakan data angka, nama tugas, mata pelajaran, dan nama siswa yang nyata dari pangkalan data aktif (appContext) di bawah. Dilarang keras mengarang (halusinasi) nilai ujian atau nama murid fiktif! Sampaikan kelebihan subjek dan nilai secara presisi sesuai kenyataan.`;
      }
...
Contoh: NAVIGATE:laporan atau NAVIGATE:absen

PANDUAN OPERASIONAL & TAB NAVIGASI APLIKASI:
Berikut adalah penjelasan singkat tab menu navigasi yang tersedia di Mode Guru:
- "dashboard": Menunjukkan ringkasan statistik (jumlah siswa, kelas, mapel), diagram grafik pencapaian nilai rata-rata per mata pelajaran dan per kelas, serta tabel peringkat siswa.
...
- "laporan": Melihat Laporan Rekapitulasi Rapor Siswa lengkap dengan nilai akhir terbobot, status kelulusan, cetak printout fisik, atau ekspor print-layout.
...
Gunakan data aktif di bawah ini untuk menjawab pertanyaan pengguna dengan valid tanpa karangan/halusinasi sedikitpun:
...
- Rekaman Penilaian / Nilai Angka: ${JSON.stringify(prunedContext.penilaian || [])}`;
      ...
      if (isQuotaExceeded) {
        console.info("[AI Copilot] Info: Free-tier Gemini quota limit reached (20 calls/day exceeded). Activating SiswaDigital high-fidelity local fallback brain smoothly.");
      } else {
        console.warn("[AI Copilot] Falling back to local brain. Reason:", error?.message || error);
      }
      
      const fallbackText = generateLocalFallbackAI(messages, prunedContext);
